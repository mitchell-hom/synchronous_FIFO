...
In the context of the testbench, this means that `RD_EN` and `WR_EN` need to be driven at different times. This is for the following reason: **They should never be sampled HIGH at the same time.** We want to be able to add a check for incorrect stimulus, so we should be able to sample both in the same packet without issue. If we drive both at the same time (at rising edge), then both will be and sampled by the monitor (blocking assignment; gated by positive edge, then updated immediately) in the beginning of the timestep and driven by at the end of the timestep (nonblocking assignment). ~~Thus, even with legal stimulus, a write followed by a read could result in a packet being sent with both `RD_EN` and `WR_EN` high at the same time.~~ *Currently, my testbench separates RD_EN and WR_EN by a half clock cycle, but I would like to be able to get away from this. I was getting an issue where the struck through text was true for some reason, which doesn't make sense to me logically. I will investigate and try to find a solution.*
